...
1. **Bổ sung transition `Đã hoàn thiện` vào A11** — Trạng thái này xuất hiện trong danh sách giá trị của `INVEST_PAYMENT_FILE_STATUS` (§B1.1) nhưng chưa có sự kiện nào dẫn tới nó trong State Machine. Cần xác định: hệ thống tự chuyển khi nào? (ví dụ: khi Maker lưu và tất cả trường bắt buộc đã điền đủ?)

2. **Bổ sung filter `PROJECT_CODE` vào B2.1** — Màn hình LIST hiện chưa có bộ lọc theo dự án/công trình, trong khi đây là trường tra cứu chính của người dùng nghiệp vụ.

3. **Làm rõ quy tắc sinh `DOCUMENT_NUMBER`** — §B1.2 ghi "hệ thống gen tự động" nhưng chưa định nghĩa format/pattern cụ thể; DEV cần rule rõ ràng để implement.

4. **(Khuyến nghị)** Thống nhất format cột A7 BIZ với các spec khác trong dự án — hiện có sự không đồng nhất giữa `PAY.OUT.MANUAL` (không có cột Mã riêng) và file này (có cột Mã BIZ-xxx).
