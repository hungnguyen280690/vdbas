---
name: gen-api-contract
description: Generate an OpenAPI 3.0.3 API contract YAML from a database SQL schema (DDL) file and a functional spec (SRS/CRUD spec) markdown file. Use when the user wants to produce an API contract from a SQL schema + SRS, or invokes /gen-api-contract with a sql_file, srs_file, and optional output_file.
---
...
If arguments are missing, ask the user for the `sql_file` and `srs_file` paths before proceeding.

---

## Instructions

Parse the arguments from the skill input.
