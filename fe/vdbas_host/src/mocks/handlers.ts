...
// Define the API Base URL (sync with your .env)
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api'
const ACL_API_BASE_URL = import.meta.env.VITE_ACL_API_BASE_URL || 'http://localhost:8082/api'
const EXP_API_BASE_URL = '/api/v1'

// ── Mock dataset for Capex Dossiers (aligned with Remote) ─────────────────────
...
const jsonResponse = (data: any, init?: any) => {
  try {
    ...
    console.error('[MSW Host] Failed to create JSON response:', err)
...
export const handlers = [
  ...
  // ── Mock User Apps API ───────────────────────────────────────────────────
  http.get(`${API_BASE_URL}/me/apps`, () => {
    console.log('[MSW Host] Intercepted GET /me/apps')
    return HttpResponse.json([
      {
        appCode: 'TEMPLATE',
        appName: 'VDBAS Template',
        appUrl: 'http://localhost:3001'
      },
      {
        appCode: 'EXP',
        appName: 'VDBAS Experience',
        appUrl: 'http://localhost:3003' 
      }
    ])
  }),

  // ── Mock Menus API (supports both /my-menus and /me/menus) ────────────────
  http.get(`${ACL_API_BASE_URL}/me/menus`, ({ request }) => {
    const url = new URL(request.url)
    const appCode = url.searchParams.get('appCode')
    console.log(`[MSW Host] Intercepted GET /me/menus for ${appCode}`)
    
    // Return menus matching either EXP or TEMPLATE code to show the menus in host
    if (appCode?.toUpperCase() === 'EXP' || appCode?.toUpperCase() === 'TEMPLATE') {
      return HttpResponse.json([
        {
          code: 'HOME',
          name: 'Trang chủ',
  ...
  http.get(`${ACL_API_BASE_URL}/my-menus`, () => {
    console.log('[MSW Host] Intercepted GET /my-menus')
    return HttpResponse.json([
      {
        code: 'HOME',
        name: 'Trang chủ',
  ...
  // ── Mock API Permissions (supports both /my-apis and /me/apis) ─────────────
  http.get(`${ACL_API_BASE_URL}/me/apis`, ({ request }) => {
    const url = new URL(request.url)
    const appCode = url.searchParams.get('appCode')
    console.log(`[MSW Host] Intercepted GET /me/apis for ${appCode}`)
    return HttpResponse.json([
      { path: '/api/category-groups', method: 'GET' },
      { path: '/api/category-groups', method: 'POST' },
      { path: '/api/category-groups/{id}', method: 'PUT' },
      { path: '/api/category-groups/{id}', method: 'DELETE' },
  ...
  http.get(`${ACL_API_BASE_URL}/my-apis`, () => {
    console.log('[MSW Host] Intercepted GET /my-apis')
    return HttpResponse.json([
      { path: '/api/category-groups', method: 'GET' },
      { path: '/api/category-groups', method: 'POST' },
      { path: '/api/category-groups/{id}', method: 'PUT' },
  ...
  // ── Mock Category Groups API (for Experience Remote) ──────────────────────
  http.post(`${EXP_API_BASE_URL}/category-groups/search`, () => {
    console.log('[MSW Host] Intercepted POST /category-groups/search (Exp)')
    return HttpResponse.json({
      content: [
        {
          id: 1,
          groupCode: 'CAT001',
          groupName: 'Mock Category Group 1',
          description: 'This is mock data from MSW (Host Intercepted)',
          active: true,
          system: false,
          deleted: false,
        },
        {
          id: 2,
          groupCode: 'CAT002',
          groupName: 'Mock Category Group 2',
          description: 'Another mock entry for testing (Host Intercepted)',
          active: true,
          system: false,
          deleted: false,
        },
  ...
  http.get(`${EXP_API_BASE_URL}/category-groups/:code`, ({ params }) => {
    console.log(`[MSW Host] Intercepted GET /category-groups/${params.code}`)
    return HttpResponse.json({
      id: 1,
      groupCode: params.code,
      groupName: `Mock Category Group ${params.code}`,
      description: 'Single record mock from Host',
      active: true,
      system: false,
      deleted: false,
    })
  ...
  http.post(`${EXP_API_BASE_URL}/category-groups`, async ({ request }) => {
    const data = await request.json() as any
    console.log('[MSW Host] Intercepted POST /category-groups', data)
    return HttpResponse.json({ ...data, id: Math.floor(Math.random() * 1000) }, { status: 201 })
  }),

  http.put(`${EXP_API_BASE_URL}/category-groups/:code`, async ({ request, params }) => {
    const data = await request.json() as any
    console.log(`[MSW Host] Intercepted PUT /category-groups/${params.code}`, data)
    return HttpResponse.json({ ...data, groupCode: params.code })
  }),

  http.delete(`${EXP_API_BASE_URL}/category-groups/:code`, ({ params }) => {
    console.log(`[MSW Host] Intercepted DELETE /category-groups/${params.code}`)
    return new HttpResponse(null, { status: 204 })
  }),

  // ── Dossier: List / Search ────────────────────────────────────────────────
  http.get(`${EXP_API_BASE_URL}/capex-dossier`, ({ request }) => {
    try {
      const url = new URL(request.url)
      console.log('[MSW Host] Intercepted GET /capex-dossier', url.search)
      ...
      console.error('[MSW Host] Error in GET /capex-dossier:', error)
      return jsonResponse({ message: 'Internal Server Error', error: String(error) }, { status: 500 })
    }
  }),

  // ── Dossier: Get Detail ───────────────────────────────────────────────────
  http.get(`${EXP_API_BASE_URL}/capex-dossier/:id`, ({ params }) => {
    try {
      console.log('[MSW Host] Intercepted GET /capex-dossier/:id', params.id)
      ...
      console.error('[MSW Host] Error in GET /capex-dossier/:id:', error)
      return jsonResponse({ message: 'Internal Server Error', error: String(error) }, { status: 500 })
    }
  }),

  // ── Dossier: Create ───────────────────────────────────────────────────────
  http.post(`${EXP_API_BASE_URL}/capex-dossier`, async ({ request }) => {
    try {
      console.log('[MSW Host] Intercepted POST /capex-dossier')
      ...
      console.error('[MSW Host] Error in POST /capex-dossier:', error)
      return jsonResponse({ message: 'Internal Server Error', error: String(error) }, { status: 500 })
    }
  }),

  // ── Dossier: Update ───────────────────────────────────────────────────────
  http.put(`${EXP_API_BASE_URL}/capex-dossier/:id`, async ({ params, request }) => {
    try {
      console.log('[MSW Host] Intercepted PUT /capex-dossier/:id', params.id)
      ...
      console.error('[MSW Host] Error in PUT /capex-dossier/:id:', error)
      return jsonResponse({ message: 'Internal Server Error', error: String(error) }, { status: 500 })
    }
  }),

  // ── Dossier: Delete ───────────────────────────────────────────────────────
  http.delete(`${EXP_API_BASE_URL}/capex-dossier/:id`, async ({ params, request }) => {
    try {
      console.log('[MSW Host] Intercepted DELETE /capex-dossier/:id', params.id)
      const body = await request.json() as any
      ...
      console.error('[MSW Host] Error in DELETE /capex-dossier/:id:', error)
      return jsonResponse({ message: 'Internal Server Error', error: String(error) }, { status: 500 })
    }
  }),

  // ── Dossier: Submit ───────────────────────────────────────────────────────
  http.post(`${EXP_API_BASE_URL}/capex-dossier/:id/submit`, ({ params }) => {
    try {
      console.log('[MSW Host] Intercepted POST /capex-dossier/:id/submit', params.id)
      const idx = MOCK_DOSSIERS.findIndex((d) => d.dossierId === params.id)
      if (idx === -1) {
        return jsonResponse(
          { errorCode: 'MSG-ERR-NOT-FOUND', message: 'Không tìm thấy hồ sơ' },
      ...
      console.error('[MSW Host] Error in POST /capex-dossier/:id/submit:', error)
      return jsonResponse({ message: 'Internal Server Error', error: String(error) }, { status: 500 })
    }
  }),

  // ── Dossier: Workflow ─────────────────────────────────────────────────────
  http.post(`${EXP_API_BASE_URL}/capex-dossier/:id/workflow`, async ({ params, request }) => {
    try {
      console.log('[MSW Host] Intercepted POST /capex-dossier/:id/workflow', params.id)
      const body     = await request.json() as any
      ...
      console.error('[MSW Host] Error in POST /capex-dossier/:id/workflow:', error)
      return jsonResponse({ message: 'Internal Server Error', error: String(error) }, { status: 500 })
    }
  }),

  // ── Master Data: Projects ─────────────────────────────────────────────────
  http.get(`${EXP_API_BASE_URL}/master-data/projects`, ({ request }) => {
    try {
      console.log('[MSW Host] Intercepted GET /master-data/projects')
      const url  = new URL(request.url)
      ...
      console.error('[MSW Host] Error in GET /master-data/projects:', error)
      return jsonResponse({ message: 'Internal Server Error', error: String(error) }, { status: 500 })
    }
  }),
]
