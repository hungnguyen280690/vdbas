...
import DynamicExtAttributes from '@/shared/components/common/DynamicExtAttributes'
import { CategoryGroup } from '../models/CategoryGroup'

interface CategoryGroupFormModalProps {
  open: boolean
  categoryGroup: any | null
  onClose: () => void
  onSuccess: () => void
}
 ...
 */
const CategoryGroupFormModal = ({ open, categoryGroup, onClose, onSuccess }: CategoryGroupFormModalProps) => {
  // 1. Translation
  const { t: translate } = useTranslation()

  // 3. Local state
  const [extAttributes, setExtAttributes] = useState<Record<string, any>>({})

  // 4. Form instance
  const [form] = Form.useForm()

  // 7. Mutations
  const createMutation = useCategoryGroups.useCreate()
  const updateMutation = useCategoryGroups.useUpdate()
