...
const CategoryGroupsPage = () => {
  ...
  // 6. Queries
  const { data, isLoading } = useCategoryGroups.useList(queryParams)

  // 7. Mutations
  const updateActiveMutation        = useCategoryGroups.useUpdateActive()
  const deleteCategoryGroupMutation = useCategoryGroups.useDelete()

  // 8. Derived data
  const categoryGroups = (data as any)?.content || (data as any)?.data || data || []
  const total          = (data as any)?.totalElements || (data as any)?.total || categoryGroups.length
  ...
  const handleTableChange: TableProps<any>['onChange'] = (newPagination, _tableFilters, sorter) => {
    setPagination((prev) => ({
      ...prev,
