...
import { useAttributeMetadata } from '../../hooks/useMetadata.js'
 ...
 *
 * @param {Object}   value            - Current attributes object
 * @param {Function} onChange         - Called with the updated attributes object
 * @param {string}   scope            - Metadata scope, e.g. 'CATEGORY', 'PROFILE'
 * @param {string[]} nonDeletableKeys - Keys the user may not remove
 */
const DynamicExtAttributes = ({ value = {}, onChange, scope, nonDeletableKeys = [] }) => {
  const { data: metadataList = [], isLoading } = useAttributeMetadata(scope)
  const [extAttributes, setExtAttributes] = useState(value)
  ...
  const renderInput = (key, attrValue) => {
