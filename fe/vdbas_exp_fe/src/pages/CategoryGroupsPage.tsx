...
const CategoryGroupsPage: React.FC = () => {
  ...
  const [inputFilters, setInputFilters] = useState<InputFilterState>({
    groupCode: '',
    groupName: '',
  })

  const [isModalOpen,          setIsModalOpen]          = useState(false)
  const [editingCategoryGroup, setEditingCategoryGroup] = useState<CategoryGroupRecord | null>(null)
  ...
  // 8. Derived data
  const categoryGroups: CategoryGroupRecord[] = data?.content || data?.data || (data as CategoryGroupRecord[]) || []
  ...
  // 10. Handlers
  const handleToggleStatus = async (record: CategoryGroupRecord, checked: boolean) => {
    try {
      await updateActiveMutation.mutateAsync({
        groupCode: record[CategoryGroup.GROUP_CODE] as string,
        isActive:  checked,
      })
    } catch (error: unknown) {
  ...
  const handleTableChange: TableProps<CategoryGroupRecord>['onChange'] = (newPagination, _tableFilters, sorter) => {
    setPagination((prev) => ({
      ...prev,
    ...
    if (sortObj.field) {
      ...
      setFilters((prev) => ({
        ...prev,
        sort: `${String(sortObj.field)},${direction}`,
      }))
    }
  }
  ...
  const handleEdit = (record: CategoryGroupRecord) => {
  ...
  const columns: TableProps<CategoryGroupRecord>['columns'] = [
    // Serial number
    {
      title:  translate('common.stt'),
      key:    'stt',
      width:  70,
      render: (_: unknown, __: CategoryGroupRecord, index: number) =>
    ...
    // Status
    {
      ...
      width:     130,
      render:    (active: boolean, record: CategoryGroupRecord) => {
    ...
    // Actions — always last, fixed right
    {
      ...
      width: 160,
      fixed: 'right',
      render: (_: unknown, record: CategoryGroupRecord) => (
        <Space>
          {/* Edit */}
          {canUpdate && (
          ...
          {canUpdate && (
            <Tooltip title={translate('common.status')}>
              <Switch
                size="small"
                checked={record[CategoryGroup.IS_ACTIVE] as boolean}
                loading={updateActiveMutation.isPending}
                onChange={(checked) => handleToggleStatus(record, checked)}
                disabled={record[CategoryGroup.DELETED] as boolean}
              />
            </Tooltip>
          )}
          ...
          {canDelete && (
            <Popconfirm
              ...
              onConfirm={() => handleDelete(record[CategoryGroup.GROUP_CODE] as string)}
              disabled={record[CategoryGroup.DELETED] as boolean}
            >
              <Button
                size="small"
