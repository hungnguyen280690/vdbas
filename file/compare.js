...
const config = {
    user: process.env.DB_USER || 'vdbas_exp',
    password: process.env.DB_PASSWORD || 'vdbas_exp',
    connectString: process.env.DB_CONNECT_STRING || '172.16.5.20:1521/vdbaspdb'
};

const sqlFilePath = process.argv[2] || './EXPENDITURE.sql';
        ...
        while ((match = createTableRegex.exec(sqlContent)) !== null) {
            ...
            while (currentPos < columnsContent.length) {
                ...
                if (char === '(') parenLevel++;
                    ...
                    columnDefinitions.push(columnsContent.substring(start, currentPos).trim());
                    start = currentPos + 1;
                }
                currentPos++;
            }
            columnDefinitions.push(columnsContent.substring(start).trim());
            for (const def of columnDefinitions) {
                if (!def) continue;
                const upperDef = def.toUpperCase();
                if (upperDef.startsWith('CONSTRAINT') || upperDef.startsWith('PRIMARY KEY') || upperDef.startsWith('FOREIGN KEY') || upperDef.startsWith('UNIQUE') || upperDef.startsWith('CHECK')) continue;
                ...
                if (colName) columns[colName] = colType;
            }
